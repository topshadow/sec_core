...
export class Fuzz {
  ...
  async sendModifiedRequest(
    ...
    const paramValues = Array.isArray(modifiedParam.value)
      ? modifiedParam.value
      : [modifiedParam.value];
    ...
    switch (modifiedParam.position) {
      ...
      case ParamType.MULTIPART:
        ...
        if (contentType?.includes("application/json")) {
          ...
          jsonBody[modifiedParam.name] = paramValues.length > 1
            ? paramValues
            : paramValues[0];
    ...
    log.debug(`fuzz:url:{url}`, { url: url.href });
  ...
  private getPathTemplate(): string {
    ...
    return templates.find((t) =>
      ...
      t.split("/").every((seg, i) =>
        seg.startsWith(":") || seg === currentPath.split("/")[i]
      )
    ) || currentPath;
  }
