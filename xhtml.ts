...
import { DOMParser ,Node,Element} from "npm:xmldom@0.6.0";
 ...
 */
export function found(xhtmlString: string, searchString: string): DetectionResult[] {
 ...
 */
export function matchBetween(srcBody: string, start: string, end: string, n: number): [number, string] {
  ...
  if (endIndex === -1 || endIndex > searchEndIndex) {
    ...
    if (n === -1) {
      ...
      // 如果限制长度，返回指定长度的内容
      return [startIndex, srcBody.slice(contentStartIndex, contentStartIndex + n)];
    }
  }
...
Deno.test('test match between', async () => {
  const testCases = [
    { src: 'Hello [world] Goodbye', start: '[', end: ']', n: -1, expected: [6, 'world'] },
    { src: 'Hello [world] Goodbye', start: '[', end: ']', n: 3, expected: [6, 'wor'] },
    { src: 'Hello [world Goodbye', start: '[', end: ']', n: -1, expected: [6, 'world Goodbye'] },
    { src: 'Hello [world Goodbye', start: '[', end: ']', n: 5, expected: [6, 'world'] },
    { src: 'Hello world Goodbye', start: '[', end: ']', n: -1, expected: [-1, ''] },
  ];

  testCases.forEach((testCase, index) => {
    const result = matchBetween(testCase.src, testCase.start, testCase.end, testCase.n);
    console.log(`Test case ${index + 1}:`);
    console.log(`Input: "${testCase.src}", start: "${testCase.start}", end: "${testCase.end}", n: ${testCase.n}`);
    console.log(`Expected: [${testCase.expected[0]}, "${testCase.expected[1]}"]`);
