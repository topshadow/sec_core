...
export async function newLocalReverseHTTPUrl(mod = 'webhook.site') {
  ...
  let response = await fetch(`https://webhook.site/token/${WEBHOOK_TOKEN}/requests/${token}`);
  return { response, token };
}
export async function haveReverseRisk(token: string) {
  if(!WEBHOOK_TOKEN){throw new Error("尚未配置环境变量 WEBHOOK_TOKEN")}
  let url = `https://webhook.site/token/${WEBHOOK_TOKEN}/requests?sorting=newest&query=url:${token}`;
  console.log(url)
...
interface DNSLogResult {
  domain: string;
  token: string;
  error: Error | null;
}

export async function newDNSLogDomain(platform: string = 'dnslog.cn'): Promise<DNSLogResult> {
  switch (platform) {
    case 'dnslog.cn':
...
async function dnslogCn(): Promise<DNSLogResult> {
  try {
    ...
    // 获取 domain
    const domainResponse = await fetch(`${baseUrl}/getdomain.php?t=0.6732314765575611`);
...
export async function checkDNSLogByToken(token: string): Promise<DNSLogRecord[]> {
  try {
    const baseUrl = 'http://dnslog.cn';
