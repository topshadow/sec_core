...
import * as str from "./str.ts";
// --- 测试用例 ---
Deno.test("空字符串返回空", () => {
  assertEquals(str.randomUpperAndLower(""), "");
});

Deno.test("非字母字符保持不变", () => {
  const input = "123!@# 你好  ";
  const result = str.randomUpperAndLower(input);
...
Deno.test("str random with param", () => {
  // 使用示例
  const template =
    `{"{{randstr(2)}}":{"@type":"java.net.InetSocketAddress","val":"{{params(reverseConnTarget)}}"}}`;
  const params = {
    reverseConnTarget:
      "\\u0065\\u0078\\u0061\\u006d\\u0070\\u006c\\u0065\\u002e\\u0063\\u006f\\u006d",
  };

  const result = str.stringsWithParam(template, params);
